...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(dir=output.parent) as temporary_name:
        ...
        core_source = (root / "src" / "browser" / "lib-impl" / "waste-libc.wat").read_text(encoding="utf-8")
        ...
        run([
            ...
            "-DWASTE_POSIX_IO", str(root / "src" / "browser" / "lib-impl" / "waste-libc-helpers.c"),
            str(root / "src" / "browser" / "lib-impl" / "waste-libc-extra.c"), "-Wl,--no-entry",
    ...
    launch = "\n".join([
        ";; Generated by src/browser/tools/build-bash-runtime.py; do not edit.",
